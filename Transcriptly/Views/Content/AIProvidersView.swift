                ...
                VStack(alignment: .leading, spacing: DesignSystem.spacingLarge) {
                    ...
                    VStack(alignment: .leading, spacing: DesignSystem.spacingMedium) {
                        ...
                        VStack(spacing: DesignSystem.spacingMedium) {
                            ...
                            ServiceSelectionRow(
                                service: .fileTranscription,
                                selectedProvider: providerManager.preferences.fileTranscriptionProvider,
                                onProviderChange: { provider in
                                    var newPrefs = providerManager.preferences
                                    newPrefs.fileTranscriptionProvider = provider
                                    providerManager.updatePreferences(newPrefs)
                                }
                            )
        ...
        VStack(alignment: .leading, spacing: DesignSystem.spacingSmall) {
            if providerType == .openai {
                ...
                HStack {
                    ...
                    Picker("", selection: .init(
                        get: { providerManager.preferences.openaiFileTranscriptionModel },
                        set: { newModel in
                            var newPrefs = providerManager.preferences
                            newPrefs.openaiFileTranscriptionModel = newModel
                            providerManager.updatePreferences(newPrefs)
                        }
                    )) {
                        ForEach(Array(OpenAIModels.fileTranscriptionModels.keys), id: \.self) { modelKey in
                            Text(OpenAIModels.fileTranscriptionModels[modelKey] ?? modelKey)
                                .tag(modelKey)
                        }
                    }
                    .pickerStyle(.menu)
