...
class ShortcutManager: ObservableObject {
    ...
    // Keep a nonisolated copy for event tap callback
    private nonisolated(unsafe) var cachedShortcuts: [ShortcutBinding] = []
    ...
    func updateShortcut(_ shortcutId: String, keyCode: Int, modifiers: NSEvent.ModifierFlags) async -> ShortcutUpdateResult {
        ...
        if let index = currentShortcuts.firstIndex(where: { $0.id == shortcutId }) {
            ...
            cachedShortcuts = currentShortcuts  // Update cached copy
    ...
    private func checkAccessibilityPermissions() -> Bool {
        return AccessibilityHelper.checkPermissions()
    }
    
    private func requestAccessibilityPermissions() {
        print("🔐 ShortcutManager: Requesting accessibility permissions...")
        let options = AccessibilityHelper.createOptionsDict(withPrompt: true)
    ...
    nonisolated private func handleKeyEvent(proxy: CGEventTapProxy, type: CGEventType, event: CGEvent) -> Unmanaged<CGEvent>? {
        ...
        if !modifiers.isEmpty {
            ...
            for shortcut in cachedShortcuts {
    ...
    nonisolated private func executeShortcutAction(_ action: ShortcutAction) {
...
struct ShortcutBinding: Codable, Identifiable {
    ...
    nonisolated var eventModifiers: NSEvent.ModifierFlags {
    ...
    nonisolated private func keyCodeToString(_ keyCode: Int) -> String {
        switch keyCode {
        // Letters
...
// Helper struct to isolate accessibility API calls
struct AccessibilityHelper: Sendable {
    ...
    static func createOptionsDict(withPrompt: Bool) -> CFDictionary {
        let key = kAXTrustedCheckOptionPrompt.takeRetainedValue() as NSString
        return [key: withPrompt] as CFDictionary
    }
}