...
extension AnyTransition {
    ...
    @MainActor static let gentleSlide = AnyTransition.asymmetric(
        insertion: .move(edge: .top).combined(with: .opacity),
        removal: .move(edge: .top).combined(with: .opacity)
    )
    
    /// Card entry transition for animated content
    @MainActor static let cardEntry = AnyTransition.asymmetric(
        insertion: .scale(scale: 0.95).combined(with: .opacity),
        removal: .scale(scale: 0.95).combined(with: .opacity)
    )
    
    /// Badge appearance transition
    @MainActor static let badgeAppear = AnyTransition.asymmetric(
        insertion: .scale(scale: 0.8).combined(with: .opacity),
        removal: .scale(scale: 0.8).combined(with: .opacity)
    )
    
    /// Sidebar transition
    @MainActor static let slideFromLeading = AnyTransition.asymmetric(
        insertion: .move(edge: .leading),
        removal: .move(edge: .leading)
    )
    
    /// Settings expansion transition
    @MainActor static let expandFromTop = AnyTransition.asymmetric(
        insertion: .move(edge: .top).combined(with: .opacity),
