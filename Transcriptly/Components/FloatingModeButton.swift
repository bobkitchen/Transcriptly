...
struct FloatingModeButton: View {
    ...
    var body: some View {
        Button(action: {
            ...
            HStack(spacing: 6) {
                ...
                Text("Float")
                    .font(UIPolishDesignSystem.Typography.floatingButton)
            }
    ...
    private var backgroundView: some View {
        RoundedRectangle(cornerRadius: 12)
            .fill(isHovered ? UIPolishDesignSystem.Colors.hoverOverlay : Color.clear)
            .overlay(
                RoundedRectangle(cornerRadius: 12)
                    .strokeBorder(
                        Color.white.opacity(isHovered ? UIPolishDesignSystem.borderOpacity : 0.05), 
                        lineWidth: 0.5
                    )
            )
    }
    ...
    var body: some View {
        Button(action: {
            ...
            Image(systemName: "pip.enter")
                .font(.system(size: 14))
                .foregroundColor(.secondaryText)
                .scaleEffect(isHovered ? UIPolishDesignSystem.Hover.iconScale : 1.0)
                .animation(UIPolishDesignSystem.Animation.standard, value: isHovered)
        }
        .buttonStyle(.plain)
...
struct FloatingModeWindowButton: View {
    ...
    var body: some View {
        Button(action: {
            ...
            .scaleEffect(isHovered ? UIPolishDesignSystem.Hover.buttonScale : 1.0)
            .animation(UIPolishDesignSystem.Animation.standard, value: isHovered)
        }
        .buttonStyle(.plain)
        .help("Enter floating recording mode")
